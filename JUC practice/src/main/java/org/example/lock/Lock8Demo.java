 ...
 *  也就是說如果一個實例對物件的同步方法獲取鎖後，該實例對象的其他同步方法必須等待獲取鎖的方法釋放鎖後才能獲取鎖
 *
 *  所有的靜態同步方法用的也是同一把鎖——類對象本身，就是我們這裡說的Phone.class
 *  具體實例對象this和類對象class是兩個不同的對象，所以他們用的是兩把不同的鎖，一個是this，一個是class
 *  但是一旦有一個執行緒獲得類對象的鎖後，無輪new出多少個實例對象，都必須等待該執行緒釋放鎖後才能獲得鎖
 *
 */
public class Lock8Demo {
    ...
    public static void main(String[] args) {
        ...
        // 嘗試修改以下方法，以phone1、phone2呼叫有不同修飾詞的方法(如1-8的案例)，觀察結果，是否符合預期筆記
        new Thread(() -> {
            phone1.sendEmail();
        }, "A").start();
